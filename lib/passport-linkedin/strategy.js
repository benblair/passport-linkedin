...
function Strategy(options, verify) {
  ...
  this.fields = options.fields || 'id,first-name,last-name';

  if (this.scope) {
    options.requestTokenURL = options.requestTokenURL + '?scope=' + this.scope;
  }
...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  ...
  this._oauth.get('https://api.linkedin.com/v1/people/~:(' + self.fields + ')?format=json', token, tokenSecret, function (err, body, res) {
